...
# =============================================================================
# LLVM Windows ARM64 Stage 2 Build — full toolchain using Stage 1 clang-cl + libc++.
#
# Uses the MSVC-built ARM64 Stage 1 clang as the host compiler to produce a
# self-hosted LLVM toolchain with libc++ as the default C++ stdlib, native
# to ARM64 Windows. This gives users a toolchain that does not require
# MSVC C++ headers/libs for C++ compilation (only Windows SDK for C and
# linking).
#
...
function Invoke-PostInstall {
    ...
    # Bundle UCRT (ARM64-native UCRT redistributable)
...
# ── Main ─────────────────────────────────────────────────────────────────────
function Main {
    Log "LLVM Windows ARM64 Stage 2 build starting"
    Log "  VARIANT:          $VARIANT"
    ...
    Log "LLVM Windows ARM64 Stage 2 build complete!"
}
